...
namespace MaxsuLogsBackUpNamespace
{
	...
	static inline const std::wstring GetBackUpLogNameByIndex(std::wstring log_path, unsigned int log_Index)
	{
		auto backfile = log_path;

		backfile += std::to_wstring(log_Index);
	...
	static inline const std::wstring GetLogAbsolutetPath(const std::filesystem::path a_fileName)
	{
		wchar_t* pathBuffer;
